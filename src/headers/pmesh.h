...
#include <cassert>
#include <stdexcept>
#include "vartypedef.h" /// typedefs for double --> Real, and int --> Int.
...
public:
        ...
        /// ------ Methods --------
        /// Default constructor
        pMesh(size_t Nx,size_t Nvars=1) :
                nx_(Nx),
                nvars_(Nvars)
        {
        ...
        */
        ...
        size_t spindex(Int i, Int q=0, Int vi=0) const
        {
                //std::cout << q << std::endl;
                assert(q>=0 && q<4*H+1);
...
pMesh<T,H>& pMesh<T,H>::operator=(const pMesh<T,H>& rhs)
{
        validateDims(rhs);
...
template <class T, Int H>
pMesh<T,H>& pMesh<T,H>::operator=(const T& rhs)
{
        for (size_t i=0;i<this->mem_.size();i++)
        {
                this->mem_[i] = rhs;
...
///--------------------------------------------------
template <class T, Int H>
inline pMesh<T, H> operator+(pMesh<T, H> lhs, const pMesh<T, H>& rhs)
{
        lhs += rhs;
        return lhs;
}
...
///--------------------------------------------------
template <class T, Int H>
inline pMesh<T, H> operator*(pMesh<T, H> lhs, const T& rhs)
{
        lhs *= rhs;
        return lhs;
}
