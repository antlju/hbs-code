...
void setup(Mesh &u, const Pencil &x, const Pencil &y, const Pencil &z)
{
	...
	for (size_t i=0;i<u.nx_;i++)
	{
		for (size_t j=0;j<u.ny_;j++)
		{
			...
			{
				u(i,j,k,0) = sin(x(i))+sin(y(j))+sin(z(k));
				//u(i,j,k,1) = sin(y(j));
				//u(i,j,k,2) = sin(z(k));
				//u(i,j,k,0) = x[i];
				//u(i,j,k,1) = y[i];
...
void apply_grad(const Mesh &u, Mesh &divu, const Real xfac, const Real yfac, const Real zfac)
{
        Bundle Dbundle(u.nx_,1); /// scalar bundle
        Pencil Dpencil(u.nx_,3); /// 3-vector pencil
        for (size_t j=0;j<u.ny_;j++)
        {
                for (size_t k=0;k<u.nz_;k++)
                {
                        ff2bundle(u,Dbundle,j,k);
                        sgrad(Dbundle,Dpencil,xfac,yfac,zfac);
...
Int main()
{
	...
	Mesh mangu(Nx,Ny,Nz,3);
		
	Mesh g(Nx,Ny,Nz,3);
	setup2(mangu,x,x,x);
	apply_grad(u,g,1.0/dx,1.0/dx,1.0/dx);
			...
			for (size_t jj=0;jj<g.ny_;jj++)
			{
				for (size_t kk=0;kk<g.nz_;kk++)
				{
					...
					else
					{
						...
						std::cout << g(ii,jj,kk,vii)/mangu(ii,jj,kk,vii) << "\t";
